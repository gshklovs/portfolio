...
export function generateMetadata(props: any): Metadata {
  ...
  const title = project
    ? `${project.title} – Greg Shklovski`
    : "Project – Greg Shklovski";
  const description = project?.description || "Project detail";
  const image = project?.image || "/img/IMG_3457-1200.jpg";
  return {
    title,
    description,
    openGraph: {
  ...
  if (!project) {
    return (
      <div className="container py-16">
        ...
        <Link
          href="/projects"
          className="mt-6 inline-block text-primary-400 hover:text-primary-300"
        >
          ← Back to projects
        </Link>
      </div>
    ...
    <div className="container py-16">
      ...
      {project.link && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-6 inline-block rounded-md bg-primary-600 px-4 py-2 font-medium text-white hover:bg-primary-500"
        >
          View external page
        </a>
      )}
